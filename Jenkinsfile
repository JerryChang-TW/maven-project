    ...
    stages{
        ...
        stage('Deploy'){
            parallel{
                stage('staging'){
                    steps{
                       sh "scp -i /opt/id_rsa **/*.war ubuntu@${params.DEV_STAGING}:/var/lib/tomcat10/webapps"
                    }
                    post{
                        success{
                ...
                stage('production'){
                    steps{
                        sh "scp -i /opt/id_rsa **/*.war ubuntu@${params.DEV_PRODUCTION}:/var/lib/tomcat10/webapps"
                    }
                    post{
                        success{
                            echo "deploy to staging clear"
